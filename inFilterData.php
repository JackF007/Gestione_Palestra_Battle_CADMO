 <table id="example2" class="table table-bordered table-striped">
     ...
     <tbody>
         <?php
            ...
            $listgraph = [];
            include 'config.php';
            $risultato = $con->query("SELECT p.id_prenotazione, DATE_FORMAT(p.data_effettuazione,\"%d-%m-%Y\") , DATE_FORMAT(p.data_appuntamento,\"%d-%m-%Y\"), p.fascia_oraria, a.nome_attivita  , p.stato_prenotazione,p.presenza, p.id_utente_prenotazione,  u.nome , u.cognome , p.str_data FROM prenotazioni as p join utenti as u on u.id_utente = p.id_utente_prenotazione join login as l on l.login_id = u.login_id  join attivita as a on p.tipo_attivita = a.id_attivita WHERE  p.data_appuntamento BETWEEN '$dal' AND '$al'AND p.stato_prenotazione='intatta'or p.stato_prenotazione='modificata' order by p.data_appuntamento desc ");
